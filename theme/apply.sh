...
/* Window, toolbars and the two panes that fill most of the screen. */
:root, body,
#messengerWindow, #tabmail, #tabpanelcontainer,
#toolbar-menubar, #tabs-toolbar, #mail-toolbar-menubar2,
#folderPaneBox, #folderTree, #threadPane, #threadTree, #messagePaneBox {
    background-color: var(--tb-bg) !important;
    color: var(--tb-fg) !important;
}

/* The folder list sits slightly back from the message list. */
#folderPaneBox, #folderTree {
    background-color: var(--tb-bg-alt) !important;
}

/* Selection and the active tab carry the accent, as everything else here does. */
...
.tabmail-tab[selected="true"] .tab-label {
    color: var(--tb-accent) !important;
}

/* Unread stands out; read mail recedes. */
