...
# Generated by theme/apply.sh. newsboat takes colour names, not hex, so this
# leans on the ANSI palette foot is already configured with.
...
# The title and hint bars. Every part of the hint bar shares one background —
# leaving hint-key on "default" let the terminal background show through in
# patches, and black on the muted blue was too low-contrast to read. Keys are
# told apart by weight, not by a second colour.
color info                black    yellow   bold
color hint-key            black    yellow   bold
color hint-keys-delimiter black    yellow
color hint-separator      black    yellow
color hint-description    black    yellow
